...
In this post I'll walk through a real testing problem I solved with a single SQL query that combines data from five different tables into one chronological timeline of events.
...
A stored procedure, `sp_RefreshCustomerReport`, handles this logic. It's triggered automatically whenever:

- A customer record is inserted or updated
- An address check row is inserted
- An identity check row is inserted

```sql
-- Core logic from sp_RefreshCustomerReport
SELECT @StatusA = ISNULL((
    SELECT TOP 1 [Status]
    FROM CustomerAddressCheck
    WHERE CustomerID = @CustomerID
    ORDER BY StatusDateTime DESC, CheckID DESC
), 0);

DECLARE @StatusI TINYINT = ISNULL((
    SELECT TOP 1 [Status]
    FROM CustomerIdentityCheck
    WHERE CustomerID = @CustomerID
    ORDER BY StatusDateTime DESC, CheckID DESC
), 0);

SET @StatusID = CASE
    WHEN @StatusA < @StatusI THEN @StatusA
    ELSE @StatusI
END;
```

The procedure only inserts a new report row if something meaningful changed (address or status), which keeps the report table lean.

## The Testing Problem

After setting up test data - inserting customers, progressing their checks through various statuses, updating an address - I ended up with data spread across five tables, each with its own timestamp column and its own idea of what "happened" to CustomerID 1.
...
You _can_ answer those questions by mentally cross-referencing five result sets and their timestamps. But that's slow, error-prone, and not something you'd want to do repeatedly across multiple customers or test runs.
...
UNION ALL
SELECT * FROM DOB WHERE (CurrentValue != PreviousValue OR PreviousValue IS NULL)
UNION ALL
-- ... all other CTEs ...
UNION ALL
...
1. **Row 1–4**: Customer inserted - all fields set for the first time in `CustomerHistory` (PreviousValue is NULL)
2. **Row 5–8**: `CustomerReport` created with matching initial values and status 0 (Not all checks started)
3. **Row 9**: Address check moves from "not started" to "Failed" (status 0 → 2)
4. **Row 10**: Identity check moves from "not started" to "Passed" (status 0 → 3)
5. **Row 11**: Report refreshes - status becomes 2 (Failed), because at least one of the status checks has failed
6. **Row 12**: Customer address updated from "12 High Street" to "12 High Road" in `CustomerHistory`
7. **Row 13**: `CustomerReport` picks up the address change
8. **Row 14**: Address check now passes (status 2 → 3)
9. **Row 21**: Report refreshes - status becomes 3 (Pass), because both address and identity checks have now passed

Every step is traceable. Every report state is explainable.

## Why This Matters for Testing

This query transforms what would be a tedious manual comparison of five separate result sets into a single, readable audit trail. It's useful for:

- **Verifying trigger behaviour** - have triggers fired correctly on every update?
- **Validating report status logic** - have the most recent address and identity statuses been uused to calculate the report status?
