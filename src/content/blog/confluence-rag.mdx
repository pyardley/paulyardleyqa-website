...
Confluence pages
      │
      ▼
  ConfluenceLoader          – fetches raw page content via the Confluence REST API
      │
      ▼
RecursiveCharacterTextSplitter  – breaks pages into overlapping chunks (1000 chars, 100 overlap)
      │
      ▼
  HuggingFace Embeddings    – converts each chunk into a vector (all-MiniLM-L6-v2, runs locally)
      │
      ▼
    ChromaDB                – stores vectors on disk in ./chroma_db
      │
      ▼  (at query time)
    Retriever               – finds the top-3 most semantically similar chunks
      │
      ▼
  Claude (Anthropic)        – reads the chunks as context and generates a grounded answer
      │
      ▼
  Answer + Source URLs
```
